...
class DatabaseConnector:
    """Handles Neo4j database connections and data extraction."""
    ...
    def __init__(self):
        ...
        if not all([self.uri, self.password]):
            raise ValueError("Missing required Neo4j environment variables: NEO4J_URI, NEO4J_PASSWORD")
    ...
    def load_existing_data(self) -> pd.DataFrame:
        """Extract all case data from Neo4j into a DataFrame with robust error handling."""
        ...
        try:
            ...
            logger.info(f"Successfully loaded {len(df)} records from Neo4j")
            return df
            
        except Exception as e:
            driver.close()
            logger.error(f"Error loading data from Neo4j: {str(e)}")
            raise
    ...
    def _clean_dataframe(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        numeric_columns = ['age', 'latitude', 'longitude', 'intervention_effectiveness', 
                          'intervention_cost', 'compliance_rate']
        for col in numeric_columns:
            if col in df.columns:
                df[col] = pd.to_numeric(df[col], errors='coerce')
        
        # Fill missing categorical values
        categorical_columns = ['gender', 'age_group', 'region', 'severity', 
                              'intervention_type']
        ...
        boolean_columns = ['is_outbreak_related', 'contact_tracing_needed', 
...
class EnhancedDiseaseSimulation:
    ...
    def create_agents(self):
        ...
        simulation_engine = Agent(
            ...
            verbose=True,
            tools=[search_internet, search_tavily]
        )
...
def main():
    ...
    try:
        ...
        df = db_connector.load_existing_data()
        ...
        disease_name = "covid19"  # Change this to your target disease
        simulation_days = 30
