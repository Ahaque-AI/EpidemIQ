...
import logging
from crewai_simulation import run
import traceback
from langchain.prompts import PromptTemplate
import json
...
@app.route("/ai_agent_simulation", methods=["GET", "POST"])
def ai_agent_simulation():
    """
    GET  → serve the HTML page.
    POST → run the simulation with JSON payload
           { disease_input: str, days_input: int }
           and return { filename: str, content: str }.
    """
    ...
    disease = data.get("disease_input")
    days    = int(data.get("days_input", 0))

    # Call your simulation runner
    filename = run(disease, days, df)          # ← your existing run() function
    if not filename or not os.path.isfile(filename):
        return jsonify(error="Simulation failed or file not found."), 500
